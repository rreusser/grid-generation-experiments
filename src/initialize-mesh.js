...
function initializeMesh (code, eta, xy, n, ratio1, ratio2) {
  ...
  var sUpper = function(x) {
    return arcLength([airfoil.xUpper, airfoil.yUpper], 0, Math.max(1e-15, x), 1e-5, 1, 20)
  }

  var sLower = function(x) {
    return arcLength([airfoil.xLower, airfoil.yLower], 0, Math.max(1e-15, x), 1e-5, 1, 20)
  }
